...
for i=1:64
    ...
    elseif  j<=4-dj, cmap(i,:) = [1 0.95 0.95]; % dermis
    elseif  j<=5-dj, cmap(i,:) = [0.8 0.3 0.3]; % epidermis
    elseif  j<=6-dj, cmap(i,:) = [f1 1 f1]; % skull
    elseif  j<=7-dj, cmap(i,:) = [f2 f2 f2]; % gray matter
    elseif  j<=8-dj, cmap(i,:) = [0.5 1 1]; % white matter
    elseif  j<=9-dj, cmap(i,:) = [1 .5 .5]*0.5; % basal layer
    elseif  j<=10-dj, cmap(i,:) = [1 0.8 0.8]; % soft tissue
    elseif  j<=11-dj, cmap(i,:) = [1 1 1]*0.75; % standard tissue
    end
end
